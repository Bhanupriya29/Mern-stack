...
const postblog = async (req, res, next) =>{
  ...
  let existinguserid;
  try{
    existinguserid= await User.findOne({email :email});
 }catch(err){
     const error  = new HttpError("Signup Failed, Please try again later", 501);
     return next(error);
 }
 if(existinguserid){
  ...
  const error  = new HttpError("User has not signed up", 600);
     return next(error);
...
try {
  ...
  res.json({'mesaage': "post created by user ${req.user.email}"
});
} catch (err) {
  console.log(err);
...
const getblog= (req,res,next) =>{
  return res.json({
    "message": 'get request by user ${req.user.email}'
  });
}
